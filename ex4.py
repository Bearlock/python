...
regexString = '[^A-Z][A-Z][A-Z][A-Z][a-z][A-Z][A-Z][A-Z][^A-Z]'
regexChar = '[a-z]'
...
# findall() returns a list of all the matches it found in the string
regexList = re.findall(regexString, searchString)
preString =''

# This loops through the regexList and strips the first and last characters
# off of each item in the list through the use of slices. Python tutorial 
# has a very good intro to slices and how to use them properly.
for things in regexList:
    things = things[1:-1]
    preString += things

# The loop above produced a string that excludes the 1st/last character
# so now it needs to be regexed again, and the list joined to produce
# the answer to the python challenge. 
solutionList = re.findall(regexChar, preString)
solution = ''.join(solutionList)

# Prints all the stuffs
print "\nRegex List: %r \n" % regexList
print "\nSolution List: %r \n" % solutionList
print "\nFinal solution: " + solution + ".html\n"
